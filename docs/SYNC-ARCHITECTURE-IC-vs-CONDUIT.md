> Current direction (2026-09-15): Jacob authorized shared Conduit libraries for IC local execution as well as remote Conduit. CC-01 converges the IC matching API and Conduit caller; [CC-02](IC-LOCAL-MAPPING.md) embeds shared text mapping in IC. Full connector/orchestrator hosting remains separate. See [current convergence note](IC-CORRELATION-CONVERGENCE.md). The historical parity plan below remains retired.
