...
/// <summary>
/// Google Workspace source — paged enumeration via pageToken. Users uses
/// Projection=Full to pull organizations/phones/relations. Group membership is
/// read via members.list per group.
/// </summary>
public sealed class GoogleWorkspaceSource : IConnectorSource
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        {
            await foreach (var g in EnumerateGroupsAsync(service, domain, pageSize, cancellationToken))
            {
        ...
        await foreach (var u in EnumerateUsersAsync(service, domain, pageSize, cancellationToken))
        {
            if (scope.MaxObjects.HasValue && emitted >= scope.MaxObjects.Value) yield break;
            emitted++;
            yield return u;
